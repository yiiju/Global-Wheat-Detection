...
def format_prediction_string(boxes, scores):
    ...
    for j in zip(scores, boxes):
        pred_strings.append("{0:.4f} {1} {2} {3} {4}".format(j[0], int(round(j[1][0])), int(round(j[1][1])), int(round(j[1][2])), int(round(j[1][3]))))
...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(output):
            ...
            result = {
            'image_id': image_id,
            'PredictionString': format_prediction_string(box.cpu().numpy(), scores)
            }
            results.append(result)
